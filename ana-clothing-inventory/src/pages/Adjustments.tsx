// Adjustments — record inventory corrections (add or remove)
...
export default function Adjustments() {
  ...
  async function handleSubmit() {
    ...
    if (!validate()) return
    const q = parseInt(qty, 10)
    const signedQty = direction === "add" ? q : -q
    try {
      await recordAdjustment(vId, signedQty, note || undefined, ref || undefined)
      setMessage({ type: "success", text: `✅ Adjustment of ${direction === "add" ? "+" : "-"}${q} units recorded.` })
  ...
  const delta = direction === "add" ? qNum : -qNum
  const projected = currentStock !== null && hasValidQty ? currentStock + delta : null
  const wouldGoNegative = direction === "remove" && currentStock !== null && hasValidQty && qNum > currentStock
    ...
    return (
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
          <Skeleton className="h-[320px] rounded-xxl" />
  ...
  return (
    <div>
      <h2 className="text-xl font-semibold text-text tracking-tight m-0">Adjustments</h2>
      <p className="mt-1 text-[13px] text-muted mb-6">Record inventory corrections</p>
              ...
              <>
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className={`flex-1 flex items-center justify-center gap-1.5 h-9 rounded-md text-sm font-medium transition-all ${
                        direction === "add"
                          ? "bg-success text-white shadow-sm border border-success"
                          : "border border-border bg-surface text-text hover:bg-black/[0.02]"
                      }`}
                    >
                      <Plus size={14} /> Add
                    </button>
                    <button
                      ...
                      className={`flex-1 flex items-center justify-center gap-1.5 h-9 rounded-md text-sm font-medium transition-all ${
                        direction === "remove"
                          ? "bg-error text-white shadow-sm border border-error"
                          : "border border-border bg-surface text-text hover:bg-black/[0.02]"
                      }`}
                    >
                ...
                {wouldGoNegative && (
                  <div className="mb-3 p-2 rounded-md bg-error/10 text-error text-xs flex gap-1.5 items-start">
                    <span>Removing {qNum} will make stock negative. Adjustment will still be recorded.</span>
                  </div>
                )}
                <div className="mb-3">
          ...
          {vId && pId && currentStock !== null && projected !== null ? (
            <Card header="Preview">
              <div className="flex items-center gap-1.5 mb-2">
                ...
                <span className="font-bold text-[15px]" style={{ color: direction === "add" ? colors.success : colors.error }}>
                  {direction === "add" ? "+" : ""}{delta} ADJUSTMENT
                </span>
              </div>
              ...
              <div className="flex justify-between p-2 bg-bg rounded-sm text-[13px] border border-border">
                ...
                <span className="font-bold" style={{ color: direction === "add" ? colors.success : colors.error }}>{projected} units</span>
              </div>
