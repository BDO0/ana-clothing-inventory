// Stock In — step-flow form using shadcn components
...
export default function StockIn() {
  ...
  async function handleSubmit() {
    ...
    const q = parseInt(qty, 10)
    try {
      await stockIn(vId, q, note || undefined, ref || undefined)
      setMessage({ type: "success", text: `✅ Stocked in ${q} units.` })
      setQty(""); setNote(""); setRef(""); setPId(""); setVId(""); setVariants([]); setCurrentStock(null); setErrors({})
    } catch (err) {
  ...
  async function handleBarcodeScan(code: string) {
    ...
    // Match scanned code against SKU or barcode field of any variant
    ...
    // Auto-select the product and variant
  ...
  return (
    <div>
      <h2 className="text-xl font-semibold text-text tracking-tight m-0">Stock In</h2>
      <p className="mt-1 text-[13px] text-muted mb-6">Record incoming stock</p>
          ...
          {vId && pId && currentStock !== null && projected !== null ? (
            <Card header="Preview">
              <div className="flex items-center gap-1.5 mb-2">
                <PackagePlus size={18} color={colors.success} />
                <span className="font-bold text-[15px] text-success">+{qNum || "?"} STOCK_IN</span>
              </div>
              ...
              <div className="flex justify-between p-2 bg-bg rounded-sm text-[13px] border border-border">
                ...
                <span className="font-bold text-success">{projected} units</span>
              </div>
            </Card>
          ) : (
